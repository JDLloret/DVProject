...
```
Here we can see first the A/B experiment differences, where they have opposite growths in relation to Dim2. Along to all groups remaining in separate clusters. Not giving any relevant information more that we know they are different
...
ggplot(pca_unscaled$x,
  ...
  theme_light()
```
This plot reinforces the previous statement, showing how the samples group by replicates and experiment, but nothing more relevant
...
ggplot(pca_scaled$x,
  ...
  theme_light()
```
Here we can see that it stays mostly the same as before
...
ggplot(pca_normalized$x,
  ...
  theme_light()
```
Here though we see that the clusters are grouped in an entirelly new way, discriminating by type of experiment, but showing clusters of different replicas, but PC2 no longer explains the same-replicate clusters
...
ggplot(pca_normalized$x,
  ...
  theme_light()
```
Here it stays the same as before
